...
class User:
    ...
    def find_renewal(self, renewal_time):
        ''' renewal_time(관심도 측정 시간)보다 
            이후인 유저 반환 '''
        print("not yet")
    
    def update_renewal(self, user_id_list):
        ''' 유저 renewal_time 갱신 '''
        # 기존에는 한 명씩 업데이트 했지만,
        # update_many() 를 통해서 id리스트를 받아서 한번에 업데이트 ㄱ
        print("not yet")

    def reset_tendency(self, user_id):
        ''' 유저 관심사 초기화 '''
        print("not yet")
